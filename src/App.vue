...
<template>
  ...
  <div v-else class="app-shell" :class="{ 'logs-expanded': logExpanded, 'logs-collapsed': logCollapsed, 'empty-logs': !displayRun }">
    ...
    <main class="main-area" ref="mainArea">
      ...
      <div class="content-scroll" v-show="!logExpanded">
        ...
        <div v-else class="secondary-page settings-page">
          ...
          <section class="form-section"><div class="section-heading"><h2>软件更新</h2></div><div class="setting-row"><div><strong>MHAutoUpdateCompilerNext <span class="version-label">v{{ info.version }}</span></strong><p>{{ updateState || (info.updaterReady ? '更新服务已配置' : '更新服务尚未配置') }}</p></div><button class="button" :disabled="busy || allRunning.length > 0" @click="update"><RefreshCw :size="15" />检查更新</button></div></section>
          <div class="about-app"><img src="/app-icon.png" alt="" /><div><strong>MHAutoUpdateCompilerNext</strong><span>MHAutoUpdateCompilerNext · 新版工作区</span></div><span>sharkgao</span></div>
        </div>
      </div>
    ...
    <div v-if="modal" class="modal-backdrop" @click.self="modal = ''"><section class="modal" :class="{ wide: modal === 'environment' || modal === 'plan' }" role="dialog" aria-modal="true" :aria-label="modal === 'new' ? '新建工作区' : modal === 'rename' ? '重命名工作区' : modal === 'environment' ? '环境检查' : modal === 'plan' ? '执行命令' : '确认操作'">
      ...
      <template v-else-if="modal === 'about'"><div class="about-dialog"><img src="/app-icon.png" alt="" /><h3>MHAutoUpdateCompilerNext <span>v{{ info.version }}</span></h3><p>MHAutoUpdateCompilerNext</p><p>Perforce 同步、UE 项目编译、TypeScript 编译与 Windows 计划任务管理。</p><p>作者：sharkgao</p><p class="muted">Tauri / Vue / TypeScript / Rust</p></div><footer><button class="button" @click="openHelp"><CircleHelp :size="15" />帮助文档</button><button class="button" @click="modal = ''">关闭</button></footer></template>
