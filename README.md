...
- 计划任务自动打开独立执行窗口，显示对应工作区、阶段状态和实时日志；支持停止，结束后保留结果窗口，不占用下一次定时触发。全部步骤成功后可自动用 Rider 2024.3.10 打开当前工作区的 `MHAGame\MHMobile.sln`。
...
6. 计划任务成功完成全部步骤后，调用设置中指定的 Rider 路径打开当前工作区的 `MHAGame\MHMobile.sln`；路径留空时自动查找 Rider 2024.3.10。失败、停止或预演不会打开 Rider。Rider 的 AI Plan 模式不能通过稳定的启动参数自动切换。
