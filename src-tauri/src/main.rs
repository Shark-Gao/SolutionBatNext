...
fn main() {
    ...
    if let Some(pos) = args
        .iter()
        .position(|a| a == "--monitor-run" || a == "--startup-error")
    {
        let dir = data_directory(&args).unwrap_or_else(|_| storage::data_dir());
        if let Err(error) = storage::configure_data_dir(dir) {
            eprintln!("{error}");
            return;
        }
    ...
    if let Some(pos) = args.iter().position(|a| a == "--run-task") {
        ...
        let result = (|| -> Result<(), String> {
            ...
            let legacy_rider_path = config.settings.rider_path.clone();
            let w = config
                .workspaces
                .into_iter()
            ...
            if check_only {
                ...
                println!(
                    "{}",
                    serde_json::json!({ "workspace": w.name, "ok": missing.is_empty(), "checks": checks })
                );
                if !missing.is_empty() {
            ...
            if result.status == "success" && !dry_run {
                runner::open_after_success(
                    &rider_workspace,
                    &job,
                    &rider_workspace.schedule.after_success,
                    &legacy_rider_path,
                );
            }
